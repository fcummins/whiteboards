...
/***
 *  FredsFreetag2
 * Derivative of Freetag, with code for tagger_identity removed
 ***/
...
class freetag {
  ...
  function freetag($options = NULL) {
    $available_options = array('debug', 'db', 'db_driver', 'db_user',
                               'db_pass', 'db_host', 'db_name',
                               'table_prefix', 'normalize_tags',
                               'normalized_valid_chars',
                               'block_multiuser_tag_on_object',
                               'append_to_integer', 'MAX_TAG_LENGTH',
                               'ADODB_DIR', 'PCONNECT'); 
    if (is_array($options)) {
   ...
   */ 
  function get_objects_with_tag($tag, $offset = 0, $limit = 100) {
    if (!isset($tag)) {
      return false;
    }		
    $db = $this->db;
   ...
   * @return An array of Object ID numbers that reference your original objects.
   */ 
  function get_objects_with_tag_all($tag) {
    if (!isset($tag)) {
      return false;
    }		
    $db = $this->db;
   ...
   * @return An array of Object ID numbers that reference your original objects.
   */
  function get_objects_with_tag_combo($tagArray, $offset = 0, $limit = 100) {
   ...
   * @return An array of Object ID numbers that reference your original objects.
   */ 
  function get_objects_with_tag_id($tag_id, $offset = 0, $limit = 100) {
    if (!isset($tag_id)) {
      return false;
    }		
    $db = $this->db;
   ...
   */ 
  function get_tags_on_object($object_id, $offset = 0, $limit = 20) {
    if (!isset($object_id)) {
      return false;
    }
    ...
    $prefix = $this->_table_prefix;
    $sql = "SELECT DISTINCT tag, raw_tag
			FROM ${prefix}freetagged_objects INNER JOIN ${prefix}freetags ON (tag_id = id)
   ...
   */ 
  function get_most_popular_tags($offset = 0, $limit = 25) {
    $db = $this->db;
   ...
   */ 
  function get_most_recent_objects($tag = NULL, $offset = 0, $limit = 25) {
    $db = $this->db;
   ...
   */ 
  function count_tags($normalized_version = 0) {
    $db = $this->db;
   ...
   */ 
  function get_tag_cloud_html($num_tags = 100, $min_font_size = 10, $max_font_size = 20, $font_units = 'px', $span_class = 'cloud_tag', $tag_page_url = '/tag/', $offset = 0) {

    $tag_list = $this->get_tag_cloud_tags($num_tags, $offset);
   ...
   */ 
  function get_tag_cloud_tags($max = 100, $offset = 0) {
    $db = $this->db;
   ...
   * @return int Returns the count
   */ 
  function count_unique_tags($normalized_version = 0) {
    return $this->count_tags($normalized_version);
  } 
  /**
   * similar_tags
   *
   ...
   *
   */ 
  function similar_objects($object_id, $threshold = 1, $max_objects = 5) {
